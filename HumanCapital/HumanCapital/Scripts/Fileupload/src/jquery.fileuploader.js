...
(function ($) {
				        ...
				        for (var i = 0; i < files.length; i++) {
				            ...
				            if (status !== true) {
				                ...
				                if (!status[2]) {
				                    ...
				                    if (status[1]) {
				                        var sStatus_ = status[1];
				                        switch (status[1]) {
				                            case 'fileSize': DialogWarningFileSize(); break;
				                            case 'filesType': DialogWarningFileType(); break;
				                            default: break;
    ...
    $.fn.fileuploader.defaults = {
        ...
        captions: {
            ...
            feedback: function (options) { return 'Browse ' + (options.limit == 1 ? 'file' : 'files') + ' to upload'; },
            //feedback2: function(options) { return options.length + ' ' + (options.length > 1 ? ' files were' : ' file was') + ' chosen'; },
            feedback2: function (options) { return 'Uploading ' + options.length + ' File(s)'; },
            ...
            errors: {
                ...
                fileSize: '${name} is too large! <br/>Please choose a file up to ${fileMaxSize}MB.',
